...
// Fetch order list with image + product name
$sql = "SELECT 
            o.id AS order_id,
            o.order_date,
            o.net_total,
...
/* Optional: small shadow for subtle lift */
...
<div class="orders-container">
    <h2 style="color:#14532d;" class="headingh2">My Orders</h2>

 <?php foreach ($orders as $row) { ?>
    <a href="order_details.php?id=<?= $row['order_id'] ?>" style="text-decoration:none;">
        <div class="order-card">
            <img src="/admin/<?= $row['image'] ?>" onerror="this.src='images/default.jpg';" alt="Product Image">

            <div class="order-info">
                <h3><?= $row['product_name'] ?></h3>
                <p><b>Order ID:</b> #<?= $row['order_id'] ?></p>
                <p><b>Date:</b> <?= date("d M Y", strtotime($row['order_date'])) ?></p>
                <p><b>Total:</b> ₹<?= number_format($row['net_total'], 2) ?></p>

                <span class="payment_badge <?= $row['payment_status'] ?>">
